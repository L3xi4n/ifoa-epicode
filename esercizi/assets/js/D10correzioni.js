...
function dice() {
  let min = 1;
  let max = 6;
  let dado = Math.floor(Math.random() * (max + 1 - min) + min);
  console.log(dado);
}
...
console.log("-esercizio 4 se vero-");
console.log(deleteOne("delete", false));
console.log("-esercizio 4 se falso-");
...
//   let frase = "Io ho 2 gatti";
//   let onlyLetters = frase.split("2");
...
console.log(onlyLetters('in casa ci sono 2 computer, e cellulari e 1 tablet'));

//soluzione2

function onlyLetters2 (stringa){
let nuovaStringa = '';
for (let i = 0; i < stringa.lenght; i++){
  let lettera = stringa[i];
  if (
...
console.log(onlyLetters2('in casa ci sono 2 computer, e cellulari e 1 tablet'));
...
console.log(isThisAnEmail2('pippo.pluto@com'));

// function isThisAnEmail(stringa){
//   const email = document.getElementById('email');
// if (!email.value.match(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/)){
//   alert("Devi indicare un indirizzo email valido");
//   return false;
// }
...
const whatDayIsIt = function () {
  ...
  const giornoSettimana = settimana[giorno];
...
const rollTheDices = (volte) => {
  for (let i = 0; i < volte; i++){
    let lancio = dice();
    dadi.sum += lancio;
    dadi.values.push(lancio);
  } 
}

rollTheDices(3); //quante volte la funzione lancia i dadi
console.log(dadi);


// function rollTheDices(randomNumero){
//   dice(6)
...
const scorsoMillennio = (array) => {
  return array.filter((film) => parseInt(film.Year) < 2000);
};

console.log("-esercizio 15-");
console.log(scorsoMillennio(movies));
